...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 text-slate-100 shadow-xl">
      ...
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-3 border-b border-slate-800/80">
        {/* BRAND LOGO */}
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab('streams')}>
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-pink-600 via-purple-600 to-amber-500 p-0.5 shadow-lg shadow-pink-500/20">
            <div className="w-full h-full bg-slate-950 rounded-[10px] flex items-center justify-center font-black text-lg text-pink-400">
              NEX
            </div>
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-base tracking-tight bg-gradient-to-r from-pink-400 via-purple-300 to-amber-300 bg-clip-text text-transparent">
                Nexora
              </span>
              <span className="px-2 py-0.5 text-[9px] font-black tracking-widest bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-full uppercase flex items-center gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {!dailyBonusClaimed ? (
            ...
            <span className="px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-bold text-[11px] rounded-xl flex items-center gap-1">
          ...
          {!authUser ? (
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-slate-800/80 hover:bg-slate-800 px-2.5 py-1.5 rounded-xl border border-slate-700 transition-all"
              >
                <img
                ...
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-3 z-50 space-y-1">
                  <div className="pb-2 border-b border-slate-800 mb-2">
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-800/80 flex items-center justify-between text-xs">
                      <span className="font-bold text-amber-400">🪙 {wallet.coins.toLocaleString()} Coins</span>
                      <span className="font-bold text-cyan-400">💎 {wallet.diamonds.toLocaleString()}</span>
                    </div>
                  </div>
                  <button
      ...
      {/* SECONDARY NAVIGATION TABS STRIP */}
      <div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center gap-1.5 overflow-x-auto no-scrollbar border-t border-slate-800/40">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          const isActive = activeTab === tab.id || (tab.id === 'streams' && activeTab === 'live');
          return (
            <button
              key={tab.id}
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap shrink-0 ${
                isActive
                  ? 'bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-indigo-500/20 text-pink-300 border border-pink-500/30 shadow-sm'
